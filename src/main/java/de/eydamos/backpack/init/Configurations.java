...
public class Configurations {
    ...
    public static boolean refreshConfig() {
        ...
        OPEN_ONLY_EQUIPPED_BACKPACK = config.get(
            Configuration.CATEGORY_GENERAL,
            "openOnlyEquippedBackpacks",
            false,
            getOpenOnlyEquippedBackpacksComment()
        ).getBoolean(false);
        RENDER_BACKPACK_MODEL = config.get(
            Configuration.CATEGORY_GENERAL,
            "renderBackpackModel",
            true,
            getRenderBackpackModelComment()
        ).getBoolean(true);
        MAX_BACKPACK_AMOUNT = config.get(
            Configuration.CATEGORY_GENERAL,
            "maxBackpackAmount",
            0,
            getMaxBackpackAmountComment(),
            0,
            36
        ).getInt();
    ...
    public static boolean featureEnabled(Features feature) {
        return config.get(Constants.CONFIG_CATEGORY_FEATURES, featureName(feature), true, feature.getConfigComment())
            .getBoolean(true) && refreshConfig();
    }
